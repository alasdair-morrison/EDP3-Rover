...
void controlCar(char cmd)
{
  ...
  switch(cmd)
  {
    case 'F': forward(duty); break;   // Move forward 前进
    case 'B': reverse(duty); break;  // Move backward 后退
    case 'L': turnLeft(duty); break;      // Turn left 左转
    case 'R': turnRight(duty); break;     // Turn right 右转
    case 'S': fullStop(); break;   // Stop 停止
